...
function updateSelectedTxtFileNames(files){
  ...
  el.innerHTML=names.map((name,index)=>
    `<div class="selectedFileRow" draggable="true" data-file-index="${index}">`+
      `<span class="selectedFileDrag" aria-hidden="true">⋮⋮</span>`+
      `<span class="selectedFileIcon" aria-hidden="true">📄</span>`+
...
function refreshTxtFiles(files){
  ...
  resetWorkState({keepSelectedFiles:true, keepCover:true});
...
function resetWorkState(options={}){
  ...
  if(fileCount) fileCount.innerHTML="TXT 여러 개 선택 가능 · 파일명 순서대로 합쳐져요";
...
selectedFileList.addEventListener("dragover",e=>{
  const row=e.target.closest("[data-file-index]");
  if(!row) return;
  e.preventDefault();
  e.stopPropagation();
...
selectedFileList.addEventListener("drop",e=>{
  const row=e.target.closest("[data-file-index]");
  if(!row || selectedFileDragIndex===null) return;
  e.preventDefault();
  e.stopPropagation();
  const targetIndex=Number(row.dataset.fileIndex);
  const fromIndex=selectedFileDragIndex;
  selectedFileDragIndex=null;
  moveSelectedTxtFile(fromIndex,targetIndex);
});

selectedFileList.addEventListener("dragend",e=>{
  const row=e.target.closest("[data-file-index]");

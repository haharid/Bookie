...
function findUncoveredTxtFiles(previewFiles,chapterIndexes){
  const items=Array.isArray(previewFiles) ? previewFiles : [];
  if(items.length<2) return [];
  const chapters=[...chapterIndexes].sort((a,b)=>a-b);
  const issues=[];

  items.forEach((item,fileIndex)=>{
    const start=item.combinedOffset;
    const sourceLines=item.keptLines;
    if(!Number.isInteger(start) || !Array.isArray(sourceLines) || !sourceLines.length) return;
    const end=start+sourceLines.length;
    const first=chapters.find(index=>index>=start && index<end);
    const name=item.fileName || `파일 ${fileIndex+1}`;
    if(first===undefined){
      issues.push(`${name}: 선택된 목차가 없음`);
    }else if(fileIndex>0 && sourceLines.slice(0,first-start).some(line=>String(line).trim())){
      issues.push(`${name}: 첫 목차 앞의 본문이 빠짐`);
    }
  });
...
async function makeEPUB(){
  ...
  if(titleInput){
    ...
    const previewApi=(typeof window!=="undefined") ? window.BookieEditableFrontPreview : null;
    const uncoveredFiles=findUncoveredTxtFiles(
      previewApi && typeof previewApi.getPreviewFiles==="function" ? previewApi.getPreviewFiles() : [],
      selectedIndexes
    );
    if(uncoveredFiles.length){
      const message="파일 내용이 빠질 수 있어 EPUB 생성을 멈췄어요. 앞부분 확인에서 해당 파일의 첫 장 제목을 목차로 선택해 주세요.\n"+
        uncoveredFiles.join("\n");
      log(message);
      alert(message);
      return;
...
<rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml"/>
    ...
    oebps.file("epub.css", epubStyleCss, {compression:"DEFLATE"});
    oebps.file("board.css", BOARD_CSS, {compression:"DEFLATE"});
    oebps.file("sns.css", SNS_CSS, {compression:"DEFLATE"});
...
<item id="boardcss" href="board.css" media-type="text/css"/>
<item id="snscss" href="sns.css" media-type="text/css"/>

...
const BookieParagraphBreakSettings=(()=>{
  ...
  function selectCandidate(index){
    ...
    const candidate=Number.isInteger(numericIndex) ? candidates[numericIndex] : "";
    if(!candidate) return null;

    const currentInput=byId("paragraphBreakCurrent");
    const replacementInput=byId("paragraphBreakReplacement");
    const existing=rules.find(rule=>rule.source===candidate);

    if(currentInput) currentInput.value=candidate;
    if(replacementInput){
      replacementInput.value=existing && existing.output!==existing.source
        ? existing.output
        : "";
    }

    const rule=existing ? {...existing} : addRuleValues(candidate,"");
    setMessage(existing
      ? `${candidate} 문구는 이미 적용 중이에요.`
      : `${candidate} 문구를 구분 기호로 바로 적용했어요.`);
    renderCandidates();
    currentInput?.focus();
    return rule;
  }
  ...
  function init(){
    ...
    candidateList?.addEventListener("click",event=>{
      ...
      if(!button) return;
      selectCandidate(button.dataset.paragraphBreakCandidate);
    });

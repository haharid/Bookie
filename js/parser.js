...
function createTocProfile(i){
  const rawAsciiSpacedLine = String(lines[i] || "")
    .replace(/^[\x20\t]+|[\x20\t]+$/g, "");
  const isDotWrappedNumber = /^#?\.[\x20\t]+\d+[\x20\t]+\.$/.test(rawAsciiSpacedLine);
  ...
  if(line.length > 80 && !isDotWrappedNumber){
    reject("too-long");
    return profile;
  }

  // A numbered chapter title may end with a period inside a long
  // parenthetical explanation. Keep the sentence filter for ordinary prose,
  // but do not discard a clearly delimited chapter heading.
  const numberedChapterHeading =
    new RegExp("^(?:#\\s*)?(?:(?:제|第)\\s*)?"+num+
      "\\s*(?:화|편|회|장|부|권|話|篇|回|章|部|卷)\\s*\\S", "i").test(line) &&
    isBlankLineAt(i-1) && isBlankLineAt(i+1);
  if(!isDotWrappedNumber && !numberedChapterHeading && line.length > 35 && /[.!?。？！…]["')\]」』”’]*$/.test(line)){
  ...
  if(isDotWrappedNumber) add(8, "dot-wrapped-number");
  ...
  if(new RegExp("^"+num+"\\s*[.．]$").test(line)) add((isBlankLineAt(i-1) && isBlankLineAt(i+1)) ? 7 : 4, "number-dot-only");

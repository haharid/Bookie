...
const BookieGameModeSettings = (()=>{
  ...
  function addRule(){
    ...
    if(!isSupportedPattern(pattern)){
      setMessage("괄호를 올바르게 입력하거나 일반:티롱처럼 고정 글자를 함께 입력해줘.",true);
      input?.focus();
      return null;
    }
  ...
  function updateControls(expanded){
    const enabledToggle=byId("enableGameMode");
    const sectionToggle=byId("gameModeToggle");
    const enabled=!!enabledToggle?.checked;
    const settings=byId("gameModeSettings");
    const section=byId("gameModeSection");
    const arrow=byId("gameModeArrow");
    ...
    sectionToggle?.setAttribute("aria-label",isExpanded ? "게임모드 설정 접기" : "게임모드 설정 열기");
    if(settings){
    ...
    if(!enabled || !isExpanded) hideCustomColorPalette();
  ...
  function toggleSection(){
    const expanded=byId("gameModeToggle")?.getAttribute("aria-expanded")==="true";
  ...
  function init(){
    ...
    sectionToggle?.addEventListener("click",event=>{
      if(event.target===byId("enableGameMode") || event.target?.closest?.("#enableGameMode")) return;
      toggleSection();
    });
    ...
    byId("enableGameMode")?.addEventListener("change",event=>{
    ...
    byId("gameModeCollapseButton")?.addEventListener("click",()=>updateControls(false));

...
(function initNumberedDotTocSupport(){
  ...
  function findFrontTocRemovalEnd(text){
    ...
    const tocIndex=sourceLines.findIndex((line,index)=>index<40 && line.trim()==="목차");
...
(function initEditableFrontPreview(){
  ...
  let lastDeletedLineAction=null;
  ...
  function removeTrailingVolumeLine(text){
    ...
    const lastLine=String(sourceLines[sourceLines.length-1] || "");
    if(!/(?:\d+\s*권|다음\s*권)/.test(lastLine)) return normalized;
  ...
  function removeFrontRepeatedTocBlock(text){
    ...
    const sourceLines=normalized.split("\n");
    const scanLimit=Math.min(50,sourceLines.length);
    let tocIndex=-1;

    for(let i=0;i<scanLimit;i++){
      if(/^(?:목차|차례)$/.test(normalizedTocLine(sourceLines[i]))){
        tocIndex=i;
        break;
      }
    }
    ...
    let firstEntry="";
    for(let i=tocIndex+1;i<scanLimit;i++){
    ...
    for(let i=firstEntryIndex;i<scanLimit;i++){
      const value=normalizedTocLine(sourceLines[i]);
      if(!value) continue;

      if(i>firstEntryIndex && value===firstEntry && nonBlankEntries>=2){
  ...
  async function processedFile(file){
    ...
    const sourceLines=cleaned ? cleaned.split("\n") : [];
  ...
  function updateUndoDeletedLineButton(){
    const button=document.getElementById("undoLastDeletedLineButton");
    if(!button) return;
    const count=lastDeletedLineAction ? lastDeletedLineAction.entries.length : 0;
    button.disabled=!count;
    button.textContent=count>1 ? `마지막 삭제 되돌리기 (${count}줄)` : "마지막 삭제 되돌리기";
  }

  function rememberDeletedLineAction(entries,selectedRefsBefore){
    const normalized=entries.filter(Boolean);
    if(!normalized.length) return;
    const deletedKeys=new Set(normalized.map(entry=>tocRefKey(entry.key,entry.sourceIndex)));
    lastDeletedLineAction={
      entries:normalized,
      selectedDeletedRefs:selectedRefsBefore.filter(ref=>deletedKeys.has(ref.refKey))
    };
    updateUndoDeletedLineButton();
  }
  ...
  function selectAutoChapterPattern(index){
    ...
    // 숫자가 없는 프롤로그/에필로그/외전/일반 제목은 자동 패턴을 만들 수 없다.
    // 이런 줄도 클릭이 막히지 않도록 해당 줄 하나를 직접 선택/해제한다.
    if(!regex){
      toggleFrontLineSelection(index);
      return;
    }
  ...
  function renderActiveFrontPreview(){
    ...
    for(let sourceIndex=0;sourceIndex<active.sourceLines.length && visibleCount<120;sourceIndex++){
      ...
      number.textContent=`L${sourceIndex+1}`;
  ...
  function renderDeleteSearchResults(){
    ...
    const deleteAll=document.getElementById("deleteSearchAllButton");
    if(!results || !summary || !deleteAll) return;

    results.replaceChildren();
    deleteAll.disabled=!deleteSearchMatches.length;
  ...
  async function deleteSearchLine(fileKeyValue,sourceIndex){
    ...
    rememberDeletedLineAction([{key:item.key,sourceIndex}],selectedRefsBefore);
    deletedSetFor(item.file).add(sourceIndex);
    await refreshAfterDeleteSearch(selectedRefsBefore);
  }

  async function deleteAllSearchMatches(){
    ...
    rememberDeletedLineAction(entries,selectedRefsBefore);
    await refreshAfterDeleteSearch(selectedRefsBefore);
  }
  ...
  async function deleteActiveLine(sourceIndex){
    ...
    const selectedRefsBefore=captureSelectedTocRefs();
    rememberDeletedLineAction([{key:active.key,sourceIndex}],selectedRefsBefore);
  ...
  async function undoLastDeletedLines(){
    const action=lastDeletedLineAction;
    if(!action || !action.entries.length) return;

    const currentRefs=captureSelectedTocRefs();
    action.entries.forEach(entry=>{
      const item=previewFiles.find(value=>value.key===entry.key);
      if(item) deletedSetFor(item.file).delete(entry.sourceIndex);
    });

    const restoreRefs=mergeTocRefs(currentRefs,action.selectedDeletedRefs);
    const restoredCount=action.entries.length;
    lastDeletedLineAction=null;
    updateUndoDeletedLineButton();
    await editableLoadPreview({keepFrontPanelOpen:true,restoreTocRefs:restoreRefs});

    if(deleteSearchQuery){
      deleteSearchMatches=collectDeleteSearchMatches(deleteSearchQuery);
      renderDeleteSearchResults();
    }
    if(typeof log==="function") log(`마지막 삭제 되돌리기: ${restoredCount}줄 복구`);
  }
  ...
  window.addEventListener("load",()=>{
    ...
    const deleteSearchAllButton=document.getElementById("deleteSearchAllButton");
    ...
    const undoLastDeletedLineButton=document.getElementById("undoLastDeletedLineButton");
    ...
    if(deleteSearchAllButton) deleteSearchAllButton.addEventListener("click",deleteAllSearchMatches);
    ...
    if(undoLastDeletedLineButton) undoLastDeletedLineButton.addEventListener("click",undoLastDeletedLines);
    updateUndoDeletedLineButton();

...
const BookieParagraphEngine = {
  ...
  protectSpecialSymbolLines(raw){
    ...
    const withProtectedAtB=lines.map((line,index)=>{
      if(line.trim()!=="@b") return line;
      const before=index>0 ? "\n" : "";
      const after=index<lines.length-1 ? "\n" : "";
      return `${before}@b${after}`;
    }).join("\n");

    return withProtectedAtB.replace(
      /\n([@\]#※▶▷◆■□★☆]+)\n/g,
      "\n\n$1\n\n"
    );
  },
  ...
  normalizeParagraphBreakRules(rules){
    const sourceRules=Array.isArray(rules) && rules.length
      ? rules
      : [{source:"***",output:"* * *",matchStars:true,isDefault:true}];

    return sourceRules.map(rule=>{
      const source=String(rule?.source || "").trim();
      const output=String(rule?.output ?? rule?.replacement ?? source).trim() || source;
      return {
        source,
        output,
        matchStars:!!rule?.matchStars
      };
    }).filter(rule=>rule.source);
  },

  paragraphBreakRuleMatches(line,rule){
    const text=String(line || "").trim();
    if(rule.matchStars) return text.replace(/\s+/gu,"")==="***";
    return text===rule.source;
  },

  markParagraphBreaks(raw,rules){
    const normalizedRules=this.normalizeParagraphBreakRules(rules);
    const breaks=[];
    const html=String(raw || "").split("\n").map(line=>{
      const rule=normalizedRules.find(item=>this.paragraphBreakRuleMatches(line,item));
      if(!rule) return line;
      const token=`[[BOOKIE_PARAGRAPH_BREAK_${breaks.length}]]`;
      breaks.push({token,output:rule.output});
      return token;
    }).join("\n");
  ...
  restoreReplyLines(html, replies, escapeFn){
    ...
    replies.forEach(reply=>{
      ...
      const safeContent = reply.content
        ...
        : "";
      const replacement = `</p><p class="bb-r">${reply.arrows}${safeContent}</p><p>`;
  ...
  restoreParagraphBreaks(html,breaks,escapeFn){
    let output=String(html || "");
    (breaks || []).forEach(item=>{
      const safeText=this.escapeHtml(item.output,escapeFn);
      output=output.replace(
        item.token,
        `</p><p style="text-align: center;">${safeText}</p><p>`
      );
    });
    return this.restoreStarBreaks(output);
  },
  ...
  build(raw, options = {}){
    ...
    const paragraphBreakResult=this.markParagraphBreaks(output,options.paragraphBreakRules);
    output = paragraphBreakResult.html;
    ...
    output = this.preserveSingleLineBreaks(output);
    output = this.restoreParagraphBreaks(output,paragraphBreakResult.breaks,options.escapeFn);

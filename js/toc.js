...
function renderToc(){
  ...
  html.push(`<div style="padding:8px 10px;border-bottom:1px solid var(--ui-line);font-size:13px;color:var(--ui-text);background:var(--ui-soft);font-weight:800;">전체 ${selectedIndexes.length}개 · 확실 ${sureCount}개 · 의심 ${maybeCount}개 · 수동 ${manualCount}개</div>`);
  ...
  for(let i=0;i<limit;i++){
    ...
    html.push(`
      <div class="tocItem"
        ...
        <input
          class="tocEdit"
          value="${esc(titleMap[idx] || lines[idx].trim())}"
